...
Gls{m&s} codes will play a critical role in lisencing GenIV reactors. In preparation for \Gls{msr}s, both the \gls{doene} and the \Gls{nrc} have identified several technical gaps in current \Gls{m&s} tools that are necessary for efficient and effective license application reviews \cite{betlzer_modeling_2019} \cite{usnrc_nonlwr_2020-1}.

Currently available software tools are do not have the capabilities to simulate \Gls{msr} specific proceses such as system inventory analysis, \ldots Sofware tools need these features in order to allow efficient and effecive reviews of a licensing application 
\
