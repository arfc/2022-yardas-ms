...
Much of our knowledge about \Gls{MSR}s come from the \Gls{MSRE} at \Gls{ORNL} in the 1960s %% cite a bunch of MSRE ORNL work.
As of writing this thesis, there are no \Gls{MSR}s currently in operation. Therefore, we must entirely rely on computational and/or surrogate models to study \Gls{MSR}s physics. The \Gls{MSBR} is a popular choice for computational models due to the availability of experimental data to compare results against %cite some MSRE studies
...
Recall in Sections
\ref{sec:molten_salt_reactors} and \ref{sec:msr_codes} we introduced the concepts of fuel depletion and removal and feed processes.
